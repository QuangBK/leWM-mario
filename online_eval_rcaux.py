"""Online goal-conditioned eval of LeWM Mario, with optional RC-aux reachability gate.

Same protocol as online_eval.py. When --rc-aux is set and the checkpoint
contains a reach_head_state, the CEM cost is gated per the paper's eq (25):
    C_RC(τ) = C_base(τ) · max(floor, 1 − λ_plan · R(τ, z_g))
where R(τ, z_g) = max_{1≤k<H} R_φ(ẑ_{t+k}, z_g, H−k).

--lambda-plan = 0 recovers the un-gated planner (isolates training-side effect).
--lambda-plan > 0 applies the reachability gate (planner-side effect).
"""
...
from rc_aux import ReachabilityHead, reachability_score
...
def cem_plan(model, history_emb, action_library, goal_emb, horizon,
    ...
    for _ in range(n_iters):
        ...
        # RC-aux soft feasibility gate (eq 25)
        if reach_head is not None and lambda_plan > 0.0 and horizon >= 2:
            rollout_pred = emb[:, history:]  # (S, horizon, D)
            R = reachability_score(reach_head, rollout_pred, goal_emb, horizon)
...
def main():
    ...
    ap.add_argument("--lambda-plan", type=float, default=0.0,
                    help="RC-aux gate coefficient (eq 25). 0 = base planner (no gate).")
    ap.add_argument("--gate-floor", type=float, default=0.1,
                    help="Floor m in max(m, 1−λ·R) — prevents zero-cost dominating all elites.")
    args = ap.parse_args()
    ...
    reach_head = None
    if "reach_head_state" in ck and args.lambda_plan > 0.0:
        ...
        print(f"loaded reach_head (h_max={reach_head.max_horizon}) "
              f"λ_plan={args.lambda_plan} gate_floor={args.gate_floor}")
    elif args.lambda_plan > 0.0:
        print(f"WARNING: --lambda-plan={args.lambda_plan} > 0 but ckpt has no reach_head_state. "
              "Running ungated.")
    else:
        print(f"running base planner (λ_plan=0, no reachability gate)")
