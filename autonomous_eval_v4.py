"""autonomous_eval_v4 = v3 (stuck detector) + past-actions tracking in CEM.

The bug v3 inherited from v2: when CEM rolls out a candidate plan, the FIRST
prediction step uses `ctx_act = pad-with-zeros` for the past `history` blocks.
That is OOD vs training (where past-action context was always real button
presses), so the predictor hallucinates optimistic futures from a zero-action
context.

Fix: track the actually-executed action blocks during the episode, and
prepend the most recent `history` of them to the CEM context instead of
zeros. Matches the training distribution exactly.
"""
...
def main():
    ...
    print(f"v4 (past-actions CEM)  label={args.label}  reward_mean={rm:.2f}  reward_std={rs:.2f}")
