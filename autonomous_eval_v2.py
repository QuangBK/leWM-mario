"""Autonomous play eval that supports both:
  - world model + separate reward head ckpt (`--reward-head`)
  - joint-finetuned ckpt (head_state stored alongside model_state in --ckpt)

Saves a few sample videos and an aggregate JSON summary. Tag the run with
`--label` so we can keep results from multiple variants side-by-side.
"""
...
def load_artifacts(args, device):
    """Returns (model, head, action_library, reward_mean, reward_std, head_hidden)."""
    ...
    if "head_state" in ck:
        # joint ckpt — joint head has 3 Linears (in→128, 128→128, 128→1) which matches n_layers=2
    ...
    # determine layer count from state dict
...
def main():
    ...
    ap.add_argument("--out-dir", default="/root/auto_videos_v2", type=Path)
