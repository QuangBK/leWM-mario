...
if [ ! -f /workspace/runs/reward_head_v3/reward_head_v3.pt ]; then
...
if [ ! -f /workspace/runs/joint_v2_score/best.pt ]; then
...
if [ ! -f /workspace/runs/eval/rh_v3/rh_v3_summary.json ]; then
  stage eval_score
...
if [ ! -f /workspace/runs/eval/joint_v2_score/joint_v2_score_summary.json ]; then
  python3 autonomous_eval_v2.py \
...
for ck_label in "joint_baseline:$JOINT_CKPT" "joint_v2_score:/workspace/runs/joint_v2_score/best.pt"; do
  ...
  if [ ! -f /workspace/runs/eval/h8_$label/${label}_h8_summary.json ]; then
    python3 autonomous_eval_v2.py \
      --ckpt "$ck" \
...
if [ ! -f /workspace/runs/vit_small/best.pt ]; then
...
if [ ! -f /workspace/runs/vit_small/lat_targets.pt ]; then
  python3 finetune_reward_head.py \
...
if [ ! -f /workspace/runs/vit_small/reward_head_v3.pt ]; then
  python3 finetune_reward_head_v3.py \
    --ckpt /workspace/runs/vit_small/best.pt \
    --dataset-root "$PRE" \
    --raw-root "$RAW" \
...
if [ ! -f /workspace/runs/vit_small_joint/best.pt ]; then
  python3 joint_finetune_v2.py \
    --ckpt /workspace/runs/vit_small/best.pt \
    --dataset-root "$PRE" \
    --raw-root "$RAW" \
...
if [ ! -f /workspace/runs/eval/vit_small_joint/vit_small_joint_summary.json ]; then
  python3 autonomous_eval_v2.py \
...
# === 9. PPO training ===
if [ ! -f /workspace/runs/ppo/ppo.zip ]; then
  stage ppo_train
...
if [ ! -d /workspace/data/ppo_full ] || [ "$(ls /workspace/data/ppo_full/*.npz 2>/dev/null | wc -l)" -lt 50 ]; then
  stage ppo_rollout
...
if [ ! -d /workspace/data/combined_blocked ] || [ "$(ls /workspace/data/combined_blocked/*.npz 2>/dev/null | wc -l)" -lt 100 ]; then
  python3 /workspace/lewm_mario_pkg/build_lewm_mario_dataset.py \
...
if [ ! -d /workspace/data/combined_precomputed ] || [ "$(ls /workspace/data/combined_precomputed/*.npz 2>/dev/null | wc -l)" -lt 100 ]; then
  python3 /workspace/lewm_mario_pkg/precompute_mario_dataset.py \
...
if [ ! -f /workspace/runs/combined_phase1/best.pt ]; then
  python3 /workspace/lewm_mario_pkg/train_mario.py \
    --dataset-root /workspace/data/combined_blocked \
...
if [ ! -f /workspace/runs/combined_phase1/lat_targets.pt ]; then
  python3 finetune_reward_head.py \
...
if [ ! -f /workspace/runs/combined_joint/best.pt ]; then
  python3 joint_finetune_v2.py \
    --ckpt /workspace/runs/combined_phase1/best.pt \
    --dataset-root /workspace/data/combined_precomputed \
    --raw-root /workspace/data/combined_full \
...
if [ ! -f /workspace/runs/eval/combined_joint/combined_joint_summary.json ]; then
  python3 autonomous_eval_v2.py \
